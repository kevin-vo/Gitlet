...
class Utils {
    ...
    /** Return the byte array from concatenating two byte arrays X and Y. */
    ...
    /**
     * Returns the byte array from concatenating a list of byte arrays LIST.
     */
    static byte[] concatenate(byte[][] list) {
        if (list.length == 1) {
            return list[0];
        } else {
            return concatenate(list[0],
                    concatenate(Arrays.copyOfRange(list, 1, list.length)));
        }
    }
